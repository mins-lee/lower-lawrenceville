...
final_addresses<-length(unique(dat_cleaned5$PRIMARYSTREET))%>%comma()

original_rows<-nrow(dat[dat$RELATIONSHIP=="Head",])%>%comma()
final_rows<-nrow(dat_cleaned5)%>%comma()

missing_moveout_original<-nrow(dat[dat$RELATIONSHIP=="Head" & is.na(dat$MOVEOUTDATE),])%>%comma()
missing_moveout_final<-nrow(dat_cleaned5[is.na(dat_cleaned5$MOVEOUTDATE),])%>%comma()
