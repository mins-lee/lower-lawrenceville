...
max_repeated_treatments_stays<-2
...
stays_in_parcels<-stays%>%
  select(address)%>%
  unique()%>%
  st_join(bnp_parcels,st_within,left=FALSE)
...
treated_stay_addresses<-stays%>%
  ...
  inner_join(combined%>%as.data.frame(),
             by="GEOID")%>%
...
treated_move_addresses<-moves%>%
  ...
  as.data.frame()%>%
  #join in bnps, match by geoid
  inner_join(combined%>%as.data.frame(),by="GEOID")%>%
...
test_run<-lm_robust(formula,data=outcome_disadvantage_change,
                    fixed_effects=t+start_hood,clusters=start_hood,se_type="stata")
summary(test_run)
```
