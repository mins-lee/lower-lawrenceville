...
#read in the bnps
bnps<-read.xlsx("Redevelopmentprojects_3.24.20.xlsx",sheet="Combined wgeocode")%>%
...
for(year in year_range){
  year_frame<-geocoded_data%>%
    ...
    select(-c(MOVEINDATE,MOVEOUTDATE,movein_year,moveout_year,full_zoning_type))
...
distances<-st_distance(bnps$geometry,
...
bnp_indices<-1:nrow(bnps)

# this function returns the event frame by iterating through each event, assigning event-time dummies to each rows
...
event_frame_function<-function(i,treated){
  ...
  event_year<-bnps[i,]$`Date-Started`%>%as.numeric()
...
test_run<-lm_robust(formula,data=for_event_study,fixed_effects=t+full_zoning_type,se_type="stata")
summary(test_run)
```
