...
distances_moves<-st_distance(apartments_commercial_bnps$geometry,
...
#calculate distances between new apartments and geocoded data
distances_disadvantages<-st_distance(apartments_commercial_bnps$geometry,
...
test_run<-lm_robust(formula,data=for_event_study_disadvantage,fixed_effects=t+start_hood+full_zoning_type,clusters=start_hood,se_type="stata")
summary(test_run)
```
...
data.frame(coefs=test_run$coefficients-for_normalization,
  ...
  geom_line()+
  geom_errorbar(aes(ymin=coefs-error,ymax=coefs+error))+
  labs(title="Event study, outcome = change in disadvantage score of moves")
