---
title: "simple regression - east lib only"
author: "Devraj Kori"
date: "4/27/2020"
...
included_neighborhoods<-c("East Liberty"
                          # , "Homewood","Homewood West", "Homewood South","Homewood North",
...
```{r regression}
#make client a character variable so it can be used for fixed effects
panel$CLIENT_ID<-as.character(panel$CLIENT_ID)
#run a simple regression
#model1: year and hood fixed effects
model_t_hood<-lm(moved_out~`Accumulated Developments`+factor(t),
                 data=panel)
# #model2: year and cluster fixed effects
# model_t_client<-lm(moved_out~`Accumulated Developments`+factor(t)+CLIENT_ID,
#                  data=panel)
#use stargazer to prep SEs
se_hood<-starprep(model_t_hood,se_type="stata")
#se_client<-starprep(model_t_client,se_type="stata",clusters=panel$CLIENT_ID)
summary(model_t_hood)
#summary(model_t_client)
#store the amount of rows that are treated
treated_rows<-nrow(panel[panel$`Accumulated Developments`>0,])
save(model_t_hood,se_hood,treated_rows,file=paste0("regression_models/",max_repeated_treatments_stays,"_treatments_",treatment_range,"_radius.Rdata"))
```

```{r regression_glm}
#run a simple regression
#model1: year and hood fixed effects
logit_t_hood<-glm(moved_out~`Accumulated Developments`+factor(t),
                 data=panel,
                 family="binomial")
#model2: year and cluster fixed effects
...
save(logit_t_hood,file=paste0("regression_models/logit_",max_repeated_treatments_stays,"_treatments_",treatment_range,"_radius.Rdata"))
```